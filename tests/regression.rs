...
#[test]
fn loose_buckler_adds_thirty_max_health() {
    // Given / When
    let hero = hero("left", 100, vec![item(ItemKind::LooseBuckler, 0, 0)]);

    // Then
    assert_eq!(hero.health(), 130);
    assert_eq!(hero.max_health(), 130);
}
...
fn buckler_effect_is_removed_on_the_tick_it_falls() {
    ...
    // Right opens 60/60 (30 base + 30 buckler) with DEFENDER_GUARD; the hit is
